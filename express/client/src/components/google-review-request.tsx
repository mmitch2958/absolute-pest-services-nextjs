...
export default function GoogleReviewRequest() {
  const { data: reviewData } = useQuery<{ reviewCount: number; rating: number }>({
    queryKey: ['/api/google-reviews'],
  });

  const reviewCount = reviewData?.reviewCount ?? 29;
  const rating = reviewData?.rating ?? 5.0;
  const nextMilestone = Math.ceil((reviewCount + 1) / 50) * 50;
      ...
      <CardContent className="text-center space-y-4">
        ...
        <div className="space-y-2">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            Help Us Reach {nextMilestone} Reviews!
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Join <strong>{reviewCount}+ happy customers</strong> who left a review. Your feedback helps us appear in Google Maps 
