...
export default async function CityServicePage({
  ...
  return (
    <>
      ...
      <section
        className="bg-gradient-to-br from-[#0f2b23] via-[#0a221c] to-emerald-900 text-white"
        aria-label="Hero"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          {/* Breadcrumb */}
          <nav className="text-sm text-[#c3d6bd] mb-5" aria-label="Breadcrumb">
          ...
          <p className="text-[#e9eddf] text-base sm:text-lg max-w-2xl mb-6">
          ...
          {/* Trust signals bar */}
          <div className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-[#dbe6d6] mb-7" aria-label="Trust signals">
            <span className="flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-[#c3d6bd]" />Licensed &amp; Insured</span>
            <span className="flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-[#c3d6bd]" />PA &amp; DE Certified</span>
            <span className="flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-[#c3d6bd]" />Free Quote</span>
            <span className="flex items-center gap-1"><Star className="w-3.5 h-3.5 text-[#173f35] fill-yellow-400" />5.0★ Rated</span>
            <span className="flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-[#c3d6bd]" />24/7 Emergency</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <a
              href={`tel:+1${telLink}`}
              className="flex items-center justify-center gap-2 bg-white text-[#0f2b23] font-bold px-6 py-3.5 rounded-lg hover:bg-[#eef1e7] transition-colors text-base sm:text-lg shadow-lg"
            >
            ...
            <Link
              href="#contact-form"
              className="flex items-center justify-center gap-2 bg-[#66845d] text-[#203d34] font-bold px-6 py-3.5 rounded-lg hover:bg-[#9fb896] transition-colors text-base sm:text-lg shadow-lg"
            >
              Get Free Estimate
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-10">
              ...
              <section aria-labelledby="service-content-heading">
                <h2 id="service-content-heading" className="text-2xl sm:text-3xl font-bold text-[#203d34] mb-5">
                  {service.name} in {city.name}, {city.state}
                </h2>

                {detailedContent ? (
                  <div className="prose prose-gray max-w-none text-[#3f5049]">
                    {detailedContent.split('\n\n').map((para, i) => {
                      if (para.startsWith('###')) {
                        return (
                          <h3 key={i} className="text-xl font-bold text-[#203d34] mt-6 mb-3">
                  ...
                  <div className="space-y-4 text-[#3f5049] leading-relaxed">
                    <p>
                    ...
                    <p>
                      ...
                      <a
                        href={`tel:+1${telLink}`}
                        className="text-[#173f35] font-semibold hover:underline"
                      >
                        {phone}
                      </a>{' '}
              ...
              {bulletPoints.length > 0 && (
                <section aria-labelledby="services-included-heading">
                  <h2 id="services-included-heading" className="text-2xl font-bold text-[#203d34] mb-5">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3" role="list">
                    {bulletPoints.map((point) => (
                      <li
                        key={point}
                        className="flex items-start gap-3 bg-white border border-[#e9eddf] rounded-xl p-4 shadow-sm"
                      >
                        <div className="w-6 h-6 bg-[#173f35] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                          <CheckCircle className="w-4 h-4 text-white" aria-hidden />
                        </div>
                        <span className="text-[#3f5049] text-sm leading-snug">{point}</span>
                      </li>
                    ))}
                  </ul>
                </section>
              ...
              {neighborhoods.length > 0 && (
                <section aria-labelledby="areas-served-heading">
                  <h2 id="areas-served-heading" className="text-2xl font-bold text-[#203d34] mb-5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {neighborhoods.map((area) => (
                      <div
                        key={area}
                        className="flex items-center gap-2.5 bg-[#eef1e7] border border-[#e9eddf] rounded-lg px-4 py-3"
                      >
                        <MapPin className="w-4 h-4 text-[#173f35] flex-shrink-0" aria-hidden />
                        <span className="text-sm font-medium text-[#1a332b]">{area}</span>
                      </div>
                    ))}
                  </div>
                  <p className="text-[#6b7b72] text-sm mt-3">
              ...
              <section aria-labelledby="why-choose-heading">
                <h2 id="why-choose-heading" className="text-2xl font-bold text-[#203d34] mb-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div
                      key={card.title}
                      className="bg-white border border-[#e9eddf] rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow"
                    >
                      ...
                      <h3 className="text-base font-semibold text-[#203d34] mb-1.5">{card.title}</h3>
                      <p className="text-[#58665e] text-sm leading-relaxed">{card.body}</p>
                    </div>
                  ))}
                </div>
              </section>
              ...
              {faqs.length > 0 && (
                <section aria-labelledby="faq-heading">
                  <h2 id="faq-heading" className="text-2xl font-bold text-[#203d34] mb-5">
                  ...
                  <div className="space-y-4" role="list">
                    {faqs.map((faq) => (
                      <details
                        key={faq.q}
                        className="group bg-white border border-[#e1e5dc] rounded-xl overflow-hidden"
                      >
                        <summary className="flex items-center justify-between gap-4 p-5 cursor-pointer list-none font-semibold text-[#203d34] text-base hover:bg-[#f7f6f0] transition-colors">
                          <span>{faq.q}</span>
            ...
            <aside className="space-y-6" aria-label="Contact and service links">
              ...
              <div
                id="contact-form"
                className="bg-[#eef1e7] rounded-2xl p-6 border border-[#e9eddf] shadow-sm sticky top-24"
              >
                <h3 className="font-bold text-[#203d34] text-lg mb-1">
                  Get {service.name} in {city.name}
                </h3>
                <p className="text-[#58665e] text-sm mb-4">Free quote · Same-day available</p>

                <a
                  href={`tel:+1${telLink}`}
                  className="flex items-center justify-center gap-2 bg-[#173f35] hover:bg-[#0f2b23] text-white font-bold px-5 py-4 rounded-xl w-full mb-3 transition-colors text-lg shadow"
                >
                  <Phone className="w-5 h-5" aria-hidden />
                  {phone}
                </a>
                <p className="text-center text-xs text-[#6b7b72] mb-4">or fill out the form below</p>

                <ServiceRequestForm defaultService={serviceSlug} />
              </div>

              {/* Other services in this city */}
              <div className="bg-white rounded-xl border border-[#e1e5dc] p-5 shadow-sm">
                <h3 className="font-bold text-[#203d34] mb-3 text-sm uppercase tracking-wide">
                  ...
                  {otherServices.map((s) => (
                    <li key={s.slug}>
                      <Link
                        ...
                        className="flex items-center gap-2 text-sm text-[#173f35] hover:text-[#0f2b23] hover:underline"
                      >
                    ...
                    {nearbyCities.map((nearbyCity) => (
                      <li key={nearbyCity.slug}>
                        <Link
                          ...
                          className="flex items-center gap-2 text-sm text-[#58665e] hover:text-[#173f35] hover:underline"
                        >
                          <MapPin className="w-3.5 h-3.5 flex-shrink-0 text-[#8b978f]" aria-hidden />
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <Link
                      key={link.href}
                      href={link.href}
                      className="text-xs bg-[#e9eddf] hover:bg-[#e1e5dc] text-[#3f5049] px-3 py-1.5 rounded-full transition-colors"
                    >
                      {link.label}
                    </Link>
                  ))}
      ...
      {/* Final CTA Banner */}
      <section className="bg-gradient-to-r from-[#0f2b23] to-[#0a221c] py-12" aria-label="Final call to action">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">
            {service.name} Problem in {city.name}? We Can Help.
          </h2>
          <p className="text-[#e9eddf] text-base sm:text-lg mb-7 max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a
              href={`tel:+1${telLink}`}
              className="flex items-center justify-center gap-2 bg-white text-[#0f2b23] font-bold px-8 py-4 rounded-lg hover:bg-[#eef1e7] transition-colors text-lg shadow"
            >
              <Phone className="w-5 h-5" aria-hidden />
              Call {phone}
            </a>
            <Link
              href="#contact-form"
              className="flex items-center justify-center gap-2 bg-[#66845d] text-[#203d34] font-bold px-8 py-4 rounded-lg hover:bg-[#9fb896] transition-colors text-lg shadow"
            >
              Get Free Quote
            </Link>
          </div>
