...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [contacts, inspections, contactsBySvc, contactsByCity, contactsDaily, contractsTotal, jobsTotal, invoicesTotal] = await Promise.all([
      sql`SELECT COUNT(*)::int AS n FROM contact_submissions WHERE created_at > NOW() - make_interval(days => ${days})`,
      sql`SELECT COUNT(*)::int AS n FROM inspection_schedules WHERE created_at > NOW() - make_interval(days => ${days})`,
      sql`
      ...
      sql`SELECT COUNT(*)::int AS n FROM job_logs WHERE job_date > NOW() - make_interval(days => ${days})`,
      sql`SELECT COALESCE(SUM(total),0)::float AS revenue, COUNT(*)::int AS n FROM invoices WHERE status IN ('paid','sent','viewed') AND created_at > NOW() - make_interval(days => ${days})`,
    ]);
