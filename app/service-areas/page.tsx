    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-blue-50">
      ...
      <section className="bg-gradient-to-r from-[hsl(132,48%,35%)] to-[hsl(132,48%,25%)] py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-[#e9eddf] mb-8 max-w-3xl mx-auto">
            {subheading}
          </p>
          <a
            href="tel:484-643-2225"
            className="inline-flex items-center bg-white text-[hsl(132,48%,35%)] px-8 py-4 text-lg font-semibold hover:bg-[#e9eddf] rounded-md transition-colors"
          >
      ...
      {/* Trust Signals */}
      <section className="py-8 bg-white border-b border-[#e9eddf]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            <div className="flex flex-col items-center gap-1">
              <Shield className="w-6 h-6 text-[hsl(132,48%,35%)]" />
              <span className="text-sm font-semibold text-[#203d34]">Licensed & Insured</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <Clock className="w-6 h-6 text-[hsl(132,48%,35%)]" />
              <span className="text-sm font-semibold text-[#203d34]">30+ Years Experience</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <Star className="w-6 h-6 text-[hsl(132,48%,35%)]" />
              <span className="text-sm font-semibold text-[#203d34]">5.0 Google Rating</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <ThumbsUp className="w-6 h-6 text-[hsl(132,48%,35%)]" />
              <span className="text-sm font-semibold text-[#203d34]">Local & Family Owned</span>
            </div>
          </div>
        </div>
      </section>
            ...
            {countyGroups.map((group) => (
              <div key={group.name}>
                <div className="flex items-center gap-3 mb-6">
                  ...
                  />
                  <h3 className="text-2xl font-bold text-[#203d34]">{group.name}</h3>
                  <a
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {group.areas.map((area) => (
                    <Link key={area.slug} href={`/service-areas/${area.slug}`}>
                      <div className={`${group.bgCard} rounded-lg border border-[#e9eddf] hover:shadow-lg transition-shadow cursor-pointer h-full p-5 flex items-center gap-3`}>
                        <div
                        ...
                        <span className="font-semibold text-[#203d34]">{area.name}</span>
                      </div>
                    </Link>
                  ))}
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={item.title} className="bg-white rounded-lg border border-[#e9eddf] hover:shadow-lg transition-shadow p-6">
                <div
                ...
                <h3 className="text-lg font-semibold text-[#203d34] mb-2">{item.title}</h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <div key={service.href} className="bg-white rounded-lg border border-[#e9eddf] hover:shadow-lg transition-shadow p-5 flex items-center gap-3">
                <div className="w-10 h-10 bg-[hsl(132,48%,35%)] rounded-full flex items-center justify-center flex-shrink-0 text-white">
                  <CheckCircle className="w-5 h-5" />
                </div>
                <div>
                  <Link href={service.href} className="font-semibold text-[#203d34] hover:text-[hsl(132,48%,35%)]">
                    {service.label}
                  </Link>
                  <p className="text-xs text-[#6b7b72] mt-0.5">{service.desc}</p>
                </div>
              </div>
            ))}
          </div>
